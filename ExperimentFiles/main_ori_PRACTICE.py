...
#######################################################################
'''-----------------------'''
present_time = .125 # time stimulus is on screen - ADJUST THIS FOR TEST - 0.075, .1, .125 
noise_time = .05       # time noise is on screen
'''-----------------------'''
#ADJUST THIS FOR TEST -ß
                                                                        # Self testing as orientation level 2:
                                                                        # 0.250 (I score 100% at lvl 2 with250ms... making faster)
                                                                        # .100 ( I score 91.6%; 58.33%)
                                                                        # .125 ( I score 75%; 100%)
                                                                        # .075 (I score 91.6%)
...
for d in range(0,2):
    for c in (1,2,3,4,5):
        img = pygame.image.load("Orientation_" + str(c) + "_Direction_" + str(d+1) + '.png')
        print("Contrast_" + str(c) + "_Direction_" + str(d+1) + '.png')
        #img = pygame.image.load("Contrast_" + str(test) + "_Direction_" + str(d+1) + '.png')
        presented_img_list.append(img)
...
def block(strength, wait_times):
    ...
    for trial in trials:
        Gabor = pygame.image.load('Orientation_' + str(trial['contrast']) + '_Direction_' + str(trial['direction']+1) + '.png')
        img_presented = 'Orientation_' + str(trial['contrast']) + '_Direction_' + str(trial['direction']+1) + '.png'
        ...
        if correctNumber == scoring: # a higher orienttID is lower orient (harder to see/better performance)
            correct = 1
        else:
            correct = 0
        if correct == 1:
...
allBlocks = ["1", "2","4", "8", "16", "32", "-1"]*2 # -1 is no sound
